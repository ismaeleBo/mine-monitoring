...
import { soilParameters } from "@/lib/constants/monitoring";
import { SOIL_QUALITY_LOCATION_LABELS } from "@/lib/constants/locations";

export function SoilQualityDashboard() {
    ...
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      <Card className="col-span-full">
        <CardHeader className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            <CardTitle>Soil Quality Overview</CardTitle>
            <CardDescription>
              Realtime soil quality monitoring across mining zones
            </CardDescription>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <LocationSelector
              locationList={SOIL_QUALITY_LOCATION_LABELS}
              location={location}
        ...
        {!error && historicalData.length > 0 && (
          <CardContent>
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {soilParameters.map(({ key, title, unit }) => (
                <MetricCard
                  key={key}
                  parameterKey={key}
                  title={title}
